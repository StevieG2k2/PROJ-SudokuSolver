...
public class Algorithm_COUNT extends ProjectAbstractAlgorithm {
  public final int GRID_SIZE = 9;
    ...
    class Grid {
        ...
        /* basic izris polja brez kandidatov */
        void printGrid(){
            for(int i = 0; i < GRID_SIZE; i++){
                for(int j = 0; j < GRID_SIZE; j++){
        ...
        void printCandidates(){
            ...
            for(int row = 0; row < GRID_SIZE; row++){
                /* loop po vrsticah kandidatov celice(3x3) */
                for(int subRow = 0; subRow < 3; subRow++){
                    /* to printne eno vrstico grida */
                    /* loop po stolpcih */
                    for(int col = 0; col < GRID_SIZE; col++){
                        Cell cell = grid[row][col];
                        if(cell.isSolved()){
                            System.out.print(numberPatterns[cell.value][subRow][0]);
                        } else {
                            /* loop po stolpcih kandidatov celice(3x3) */
                            for(int subCol = 0; subCol < 3; subCol++){
                                int candidate = subRow * 3 + subCol + 1;
                                /* ce celica vsebuje kandidate 123, 456, 789 */
  ...
  protected Output execute(Input input) {
    ...
    while(grid.getCandidateCount() > 0){
        cc = grid.getCandidateCount();
        fillSingles(grid);
        fillHiddenSingles(grid);
        elimNakedPairs(grid);
        ...
        if(cc == grid.getCandidateCount()) break;
    }
    String niz = "";
    for(int i = 0; i < 9; i++){
      for(int j = 0; j < 9; j++){
